...
${associationOwner._getAssociationDebugList()}`);
  ...
  static async sync(options) {
    ...
    let tableExists;
    if (options.force) {
      await this.drop({ ...options, cascade: this.sequelize.dialect.supports.dropTable.cascade || void 0 });
      tableExists = false;
    } else {
    ...
    await Promise.all(options.include.map(async (include) => {
      if (!include.separate) {
        return await Model._findSeparate(
          ...
          {
            ...(0, import_omit.default)(options, "include", "attributes", "order", "where", "limit", "offset", "plain", "scope"),
  ...
  static async findOne(options) {
    ...
    return await Model.findAll.call(this, (0, import_defaults.default)(options, {
      model: this,
      plain: true
    }));
  }
    ...
    const recursiveBulkCreate = async (instances2, options2) => {
      ...
      if (options2.individualHooks) {
        ...
        if (options2.include && options2.include.length > 0) {
          await Promise.all(options2.include.filter((include) => include.association instanceof import_associations.BelongsToAssociation).map(async (include) => {
            ...
            const includeOptions = (0, import_defaults.default)(
              (0, import_omit.default)((0, import_object.cloneDeep)(include), ["association"]),
              {
      ...
      if (options2.include && options2.include.length > 0) {
        await Promise.all(options2.include.filter((include) => !(include.association instanceof import_associations.BelongsToAssociation || include.parent && include.parent.association instanceof import_associations.BelongsToManyAssociation)).map(async (include) => {
          const associationInstances = [];
          const associationInstanceIndexToInstanceMap = [];
          for (const instance of instances2) {
  ...
  static async update(values, options) {
    ...
    const modelDefinition = this.modelDefinition;
    options = this._paranoidClause(this, (0, import_defaults.default)(options, {
      validate: true,
      hooks: true,
      individualHooks: false,
      returning: false,
    ...
    if (options.fields && Array.isArray(options.fields)) {
      ...
      options.fields = (0, import_intersection.default)(Object.keys(values), Array.from(modelDefinition.physicalAttributes.keys()));
...
- instance methods "destroy" and "restore": use the static "destroy" and "restore" methods instead.
  ...
  set(key, value, options) {
    ...
    if (!options.raw && attributeDefinition?.set) {
      ...
      (value instanceof import_base_sql_expression.BaseSqlExpression || value != null && attributeType && attributeType instanceof import_data_types.AbstractDataType && !attributeType.areValuesEqual(value, originalValue, options) || (value == null || !attributeType || !(attributeType instanceof import_data_types.AbstractDataType)) && !(0, import_isEqual.default)(value, originalValue))) {
        this._previousDataValues[key] = originalValue;
        this.changed(key, true);
      }
      this.dataValues[key] = value;
  ...
  async save(options) {
    ...
    if (!options.fields) {
      if (this.isNewRecord) {
        ...
        options.fields = (0, import_intersection.default)(this.changed(), Array.from(modelDefinition.attributes.keys()));
      }
    ...
    if (options.fields.length > 0 && this.isNewRecord && this._options.include && this._options.include.length > 0) {
      await Promise.all(this._options.include.filter((include) => include.association instanceof import_associations.BelongsToAssociation).map(async (include) => {
    ...
    if (wasNewRecord && this._options.include && this._options.include.length > 0) {
      await Promise.all(
        this._options.include.filter((include) => !(include.association instanceof import_associations.BelongsToAssociation || include.parent && include.parent.association instanceof import_associations.BelongsToManyAssociation)).map(async (include) => {

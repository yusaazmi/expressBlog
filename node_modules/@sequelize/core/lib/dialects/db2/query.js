...
class Db2Query extends import_query.AbstractQuery {
  ...
  async _run(connection, sql, parameters) {
    ...
    const params = [];
    if (parameters) {
      (0, import_forOwn.default)(parameters, (value, key) => {
  ...
  formatError(err, conn, parameters) {
    ...
    if (match && match.length > 0) {
      ...
      const constraintData = data && data.length > 0 ? data[1] : void 0;
  ...
  handleInsertQuery(results, metaData) {
    ...
    const id = results?.[0][this.getInsertIdField()] ?? metaData?.[this.getInsertIdField()] ?? results?.[0][autoIncrementAttribute.columnName];

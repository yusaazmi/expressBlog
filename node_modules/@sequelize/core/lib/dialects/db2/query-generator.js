  ...
  attributeToSQL(attribute, options) {
    ...
    if (attribute.type instanceof DataTypes.ENUM) {
      ...
      template2 += ` CHECK (${this.quoteIdentifier(attribute.field)} IN(${attribute.type.options.values.map((value) => {
        return this.escape(value, void 0, { replacements: options?.replacements });
      }).join(", ")}))`;
    } else {
    ...
    if (attribute.type !== "TEXT" && attribute.type._binary !== true && (0, import_query_builder_utils.defaultValueSchemable)(attribute.defaultValue, this.dialect)) {
      template2 += ` DEFAULT ${this.escape(attribute.defaultValue, { replacements: options?.replacements, type: attribute.type })}`;
    }

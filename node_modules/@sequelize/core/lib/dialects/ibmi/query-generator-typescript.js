...
class IBMiQueryGeneratorTypeScript extends import_query_generator.AbstractQueryGenerator {
  #internals;
  constructor(dialect, internals = new import_query_generator_internal.IBMiQueryGeneratorInternal(dialect)) {
  ...
  listSchemasQuery(options) {
    return (0, import_join_sql_fragments.joinSQLFragments)([
      ...
      options?.skip && Array.isArray(options.skip) && options.skip.length > 0 ? `AND SCHEMA_NAME NOT IN (${options?.skip.map((schema) => this.escape(schema)).join(", ")})` : ""
    ]);
  }
  ...
  listTablesQuery(options) {
    return (0, import_join_sql_fragments.joinSQLFragments)([
      ...
      options?.schema ? `AND TABLE_SCHEMA = ${this.escape(options.schema)}` : `AND TABLE_SCHEMA NOT LIKE 'Q%' AND TABLE_SCHEMA NOT LIKE 'SYS%'`,

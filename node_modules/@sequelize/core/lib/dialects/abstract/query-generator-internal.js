...
class AbstractQueryGeneratorInternal {
  ...
  getTechnicalDatabaseNames() {
    return import_object.EMPTY_ARRAY;
  }
...
\u279C literal(${JSON.stringify(sql)})`);
  ...
  formatAttribute(piece, options) {
    ...
    const columnName = model?.modelDefinition.getColumnNameLoose(piece.attributeName) ?? piece.attributeName;
  ...
  formatCast(cast, options) {
    ...
    const castSql = (0, import_where_sql_builder.wrapAmbiguousWhere)(cast.expression, this.queryGenerator.escape(cast.expression, { ...options, type }));

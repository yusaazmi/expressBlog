...
class MsSqlQueryGeneratorTypeScript extends import_query_generator.AbstractQueryGenerator {
  #internals;
  constructor(dialect, internals = new import_query_generator_internal.MsSqlQueryGeneratorInternal(dialect)) {
  ...
  listTablesQuery(options) {
    return (0, import_join_sql_fragments.joinSQLFragments)([
      ...
      options?.schema ? `AND s.name = ${this.escape(options.schema)}` : `AND s.name NOT IN (${this.#internals.getTechnicalSchemaNames().map((schema) => this.escape(schema)).join(", ")})`,

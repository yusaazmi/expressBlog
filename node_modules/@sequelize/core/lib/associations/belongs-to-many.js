...
Add your own primary key to the through model, on different attributes than the foreign keys, to be able to use this option.`);
  ...
  static associate(secret, source, target, options, pair, parent) {
    return (0, import_helpers.defineAssociation)(BelongsToManyAssociation, source, target, options, parent, normalizeBelongsToManyOptions, (newOptions) => {
  ...
  async add(sourceInstance, newInstancesOrPrimaryKeys, options) {
    ...
    let currentRows = import_object.EMPTY_ARRAY;
  ...
  async #updateAssociations(sourceInstance, currentThroughRows, newTargets, options) {
    ...
    const foreignKey = this.foreignKey;
    const otherKey = this.otherKey;
    const defaultAttributes = options?.through || import_object.EMPTY_OBJECT;
    const promises = [];
...
function normalizeBelongsToManyOptions(type, options, source, target) {
  ...
  return (0, import_helpers.normalizeBaseAssociationOptions)(type, {
    ...
    throughAssociations: options?.throughAssociations ? (0, import_object.removeUndefined)(options.throughAssociations) : import_object.EMPTY_OBJECT
  }, source, target);
}

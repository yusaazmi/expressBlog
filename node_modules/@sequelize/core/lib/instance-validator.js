...
class InstanceValidator {
  constructor(modelInstance, options) {
    ...
    if (options.fields && !options.skip) {
      options.skip = (0, import_difference.default)(Array.from(modelInstance.constructor.modelDefinition.attributes.keys()), options.fields);
    } else {
      options.skip ??= [];
    }
  ...
  _validateSchema(attribute, attributeName, value) {
    if (attribute.allowNull === false && value == null) {
      const association = Object.values(this.modelInstance.constructor.associations).find((association2) => association2 instanceof import_belongs_to.BelongsToAssociation && association2.foreignKey === attribute.fieldName);
      if (!association || !this.modelInstance.get(association.as)) {
        ...
        const errMsg = (0, import_get.default)(validators, "notNull.msg", `${this.modelInstance.constructor.name}.${attributeName} cannot be null`);
